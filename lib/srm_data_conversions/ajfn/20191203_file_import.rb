...
class CustomImport
	...
	FILE = {
		...
		# 	},
		# },
		# '20191203_AJFN_Oct_30_Pledges.csv' => {
		# 	import: true,
		# 	dedupe: true,
		# 	mappings: {
		# 		interaction: {
		# 			datetime: '2019-10-30 00:00:00 +0000',
		# 		},
		# 		supporter: {
		# 			email:      'Email',
		# 			first_name: 'First name',
		...
		# 		donation: {
		# 			value: 'Amount',
		# 			project_name: 'Event 2019',
		# 		},
		# 		flags: {
		# 			events: {
		# 				:'Oct 30th'        => true,
		# 				:'Oct 30th Pledge' => true,
		# 			},
		# 		},
		# 	},
		# },
		'20191203_AJFN_Charidy.csv' => {
			import: true,
			dedupe: true,
			mappings: {
				interaction: {
					datetime: '2019-11-04 00:00:00 +0000',
				},
				supporter: {
					email:         'Email',
					first_name:    'First Name',
...
class CustomImport
	...
	INTERNATIONAL_PHONE_CODES = {   # Add countries to this hash as needed (https://countrycode.org/)
		'brazil'        => '55'  ,
		'canada'        => '1'   ,
		'israel'        => '972' ,
		'united states' => '1'   ,
	...
	def interaction(row, conf, _, _)
		...
		# Note: a sub-interaction can be created via interaction.create_effect(description: 'whatever', datetime: Time.now/whatever)
	...
	def supporter(row, conf, interaction, _)
		...
		row.values_at(*conf[:unknown_phone]).select { |x| /\d/ === x }.each do |phone_number|
			...
			# Add "+" and country code to international number if needed.
			if pn.present? && @country.present? && @country != 'australia' && !pn.start_with?('+')
				country_code = INTERNATIONAL_PHONE_CODES[@country]
				before_pn    = pn
				
				if country_code.present?
					pn = "#{country_code}#{pn}" if !pn.start_with?(country_code)   # add country code if it's not there
					pn = "+#{pn}"                                                  # add "+" for international
				end
				puts "Phone #{before_pn} modified to be #{pn} for #{@country}."
			end
	...
	def early_project
		@project_mutex.synchronize do
			@early_project ||= 
				Projects::Data::State::Project.find_or_create(name: "Initial data import", description: "") do |x|
	...
	def use_project(name, with_interaction)
		@projects ||= {}
		@project_mutex.synchronize do
			@projects[name] ||= 
				Projects::Data::State::Project.find_or_create(name: name) do |x|
					x.description = name
					x.type        = Reference::Data::State::Type.first!(name: 'FUNDRAISING', table_name: 'project_state.projects')
					x.interaction = with_interaction
				end
		end
	end
	...
	def donation(row, conf, interaction, supporter)
		...
		DB.transaction do
			sp = Projects::Data::History::SourceProject.new(
				interaction: interaction,
				project:     use_project(conf[:project_name], interaction),
			)
			...
			transmission = Suku::Payments::Data::State::PaymentTransmission.new(
				...
				gateway: 'LEGACY',          # LEGACY means the payment will not be processed.
				interaction: interaction
			)
	...
	def import_all
		...
		@files.each do |fn, conf|
			...
			get_data(fn).each_with_index do |row, i|
				...
				country_column = conf.dig(:mappings, :address, :country) 
				@country       = country_column ? row[country_column]&.downcase : nil  # for international phone setup
				puts @country if @country.present? && @country != 'australia' # SLSLSL
...
# Surround with a DB transaction until import is working fairly well, as in most transactions successful.
# Remove the DB transaction to let the ones through that are working, and just report the ones that failed.
DB.transaction(rollback: :always) do
	CustomImport.new.import_all
end
